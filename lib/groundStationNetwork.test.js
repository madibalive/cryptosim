...
test('closest station', () => {
  ...
  expect(closest.id()).toBe('one');
});

test('closest station with stations outside field of view', () => {
  const universe = testUtils.createTestUniverse();
  const sat = new Satellite(universe, 1, 'crypto1', testUtils.ISS_TLE[0],
      testUtils.ISS_TLE[1]);
  const pos = sat.getPosition();
...
test('serialize and deserialize', () => {
  ...
  expect(GroundStationNetwork.deserialize(
      universe, gsnetwork.serialize())).toEqual(gsnetwork);
});
...
test('save and load network', () => {
  ...
  gsnetwork.addStation(gstation1);
  gsnetwork.addStation(gstation2);
  expect(GroundStationNetwork.load(universe2, gsnetwork.save())).toEqual(
      gsnetwork);
  expect(() => {
    GroundStationNetwork.load(universe2, blob);
  }).toThrow(Error);
});
