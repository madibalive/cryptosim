...
class delay_encryption {
   ...
   delete_keypair(id) {
        console.log(id.keypair_id);
        for(let i = 0; i < this.number_of_keypairs; i++) {
            if(this.database[i].id == id.keypair_id){
   ...
   fetch_public_key(id) {
       ...
       return {"status": "keypair_fetch_failed"};
   }
   ...
   fetch_private_key(id) {
        for(let i = 0; i < this.number_of_keypairs; i++){
           if(this.database[i].id == id){
               ...
               return {"time_to_release": this.database[i].time_to_release};
           }
       }

       return {"status": "key_fetch_failed"};
   }
}

export {delay_encryption};