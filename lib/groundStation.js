...
import SimulatedClock from './clocks/simulatedClock';
...
class GroundStation {
  ...
  /**
   * Compute the angle between the ground station and an arbitrary position
   * @param {GeoCoordinates} pos - A position in earth or space.
   * @return {LookAngle} the angle between the ground station and the given
   *         position.
   */
  angleToPosition(pos) {
    return core.angleBetween(this.#position, pos);
  }
   ...
   * @return {Number} 1 if transmit succeed, 0 if not. 
   */
   transmit(msg) {
    return this.#universe.transmitFromStation(this, msg);
  }
   ...
   *        message is received.
   * @param {Number} timeout - the number of milliseconds to listen to if not
   *        explicitly stopped.
   ...
   *         new messages.
   */
  startListening(callback, timeout = 60 * 60 * 1000) {
    const listenId = this.#nextListenId;
    this.#nextListenId += 1;
    this.#callbacks[listenId] = callback;
    this.#universe.clock().callbackIn(timeout, () => {
      this.stopListening(listenId);
    });
    return listenId;
  }
