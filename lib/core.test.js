...
test('angle to observer within view of satellite', () => {
  ...
  const satrec = core.createSatelliteRecord(testUtils.ISS_TLE[0],
      testUtils.ISS_TLE[1]);
  const pos = core.propagate(satrec, clock);
  pos.altitude = 0;
  pos.latitude += 1;
...
test('angle between two positions', () => {
  const clock = new SimulatedClock(new Date(2020, 1, 1, 0, 0, 0, 0));
  const satrec = core.createSatelliteRecord(testUtils.ISS_TLE[0],
      testUtils.ISS_TLE[1]);
  const groundPos = core.propagate(satrec, clock);
  groundPos.altitude = 0;
  clock.advance(15 * 60 * 1000);
  const spacePos = core.propagate(satrec, clock);
  const angleTo = core.angleTo(satrec, clock, groundPos);
  const angleBetween = core.angleBetween(groundPos, spacePos);
  expect(angleTo.azimuth).toBeCloseTo(angleBetween.azimuth, 5);
  expect(angleTo.elevation).toBeCloseTo(angleBetween.elevation, 5);
  expect(angleTo.distance).toBeCloseTo(angleBetween.distance, 5);
});
