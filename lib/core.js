...
import satellite from 'satellite.js/lib/index';
import GeoCoordinates from './geoCoordinates';
import LookAngle from './lookAngle';

/**
 * Create a satellite record.
 * @param {String} tleLine1 - the first line in the satellite's two
 *         line element set.
 * @param {String} tleLine2 - the second line in the satellite's two line
 *         element set.
 * @return {Object} an opaque object used to capture the state of a satellite's
 *         orbit to be used by functions in this module.
 */
...
exports.angleBetween = function(groundPos, spacePos) {
  const groundGeodetic = {
    longitude: satellite.degreesToRadians(groundPos.longitude),
    latitude: satellite.degreesToRadians(groundPos.latitude),
    height: groundPos.altitude,
  };

  const spaceGeodetic = {
    longitude: satellite.degreesToRadians(spacePos.longitude),
    latitude: satellite.degreesToRadians(spacePos.latitude),
    height: spacePos.altitude,
  };

  const spaceEcf =satellite.geodeticToEcf(spaceGeodetic);
  const lookAngles = satellite.ecfToLookAngles(groundGeodetic, spaceEcf);
  return new LookAngle(
      lookAngles.azimuth,
