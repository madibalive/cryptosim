...
import GroundStationNetwork from "../groundStationNetwork";
import testUtils from "../testUtils";
import binary from "../binary";
import { encrypt_message } from "../private_voting";
...
test("public random", () => {
  ...
  expect(result.timestamp).toBeLessThan(after);
  expect(
    nacl.sign.detached.verify(result.publicRandom, result.signature, publicKey)
  ).toBe(true);
});
...
test("timestamp", () => {
  ...
  expect(
    nacl.sign.detached.verify(timestamp, result.signature, publicKey)
  ).toBe(true);
});
...
test("signature", () => {
  ...
  expect(
    nacl.sign.detached.verify(msg, result.signature, satelliteSigningKey)
  ).toBe(true);
});
...
test("initialize ballot", () => {
  const [client, service, sat, universe] = setupClient();
  const pubkey = client.ballot.init(1);
  expect(pubkey).toBeTruthy();
  expect(typeof pubkey).toBe("string");
});

test("encrypt and submit vote", () => {
  const [client, service, sat, universe] = setupClient();
  const pubkey = client.ballot.init(1);
  const encryptedVote = encrypt_message(pubkey, "candidate-1");
  expect(encryptedVote).toBeTruthy();
  expect(typeof encryptedVote).toBe("string");
  expect(() => {
    client.ballot.vote(encryptedVote);
  }).not.toThrow();
});

test("finalize ballot", () => {
  const [client, service, sat, universe] = setupClient();
  const pubkey = client.ballot.init(1);
  const encryptedVote = encrypt_message(pubkey, "candidate-1");
  client.ballot.vote(encryptedVote);
  const result = client.ballot.finalize();
