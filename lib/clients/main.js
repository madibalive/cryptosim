...
import { Ballot } from "../private_voting";
...
class MainClient {
  ...
  /**
   * Provides an interface for creating, casting, and finalizing encrypted ballots
   * in a secure voting system. The ballot object is responsible for managing the
   * voting process, including generating a public key, accepting encrypted votes,
   * and finalizing the results.
   */

  ballot = {
    init: (k) => {
      this.#ballot = Ballot.new(k);
      const pubkey = this.#ballot.get_pubkey_pem();
      return pubkey;
    },
    vote: (encrypted_vote) => {
      this.#ballot.vote(encrypted_vote);
    },
    finalize: () => {
      const result = this.#ballot.finalize();
      // this.#ballot.free(); // not sure if this place to call
      return result;
    },
    free: () => {
      this.#ballot.free();
